...
**/
std::vector<glm::vec3> function (int x1, int y1, int x2, int y2){
  ...
  if(x_dominant){
      ...
      for(;;){
          pixels.push_back(glm::vec3(float(x), float(y), 0.0f));
          if ( x == x2) return pixels;
        ...
        while (!glfwWindowShouldClose(Window)) {
            try {
                if (NeedsUpdate) {
                    ...
                    glUniform1f(dotvertexpointsize, 1.0f * PointSize);
