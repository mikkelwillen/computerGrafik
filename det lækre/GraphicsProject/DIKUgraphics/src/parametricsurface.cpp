...
void ParametricSurface::SampleSurface()
{
    ...
    float delta_u = (this->umax - this->umin) / this->M;
    float delta_v = (this->vmax - this->vmin) / this->N;

    glm::vec3 V_lower_left;
    glm::vec3 N_lower_left;

    glm::vec3 V_lower_right; 
    glm::vec3 N_lower_right;

    glm::vec3 V_upper_right;
    glm::vec3 N_upper_right;

    glm::vec3 V_upper_left;
    glm::vec3 N_upper_left;
    ...
    for (int m = 1; m <= this->M; ++m)
    {
        ...
        for (int n = 1; n <= this->N; ++n)
        {
            ...
            if (this->frontfacing) {
                this->CreateFrontFacingData(V_lower_left, V_lower_right, V_upper_right, V_upper_left, this->vertices,
                                            N_lower_left, N_lower_right, N_upper_right, N_upper_left, this->normals);
            } else {
                this->CreateBackFacingData(V_lower_left, V_lower_right, V_upper_right, V_upper_left, this->vertices,
                                           N_lower_left, N_lower_right, N_upper_right, N_upper_left, this->normals);
            }
